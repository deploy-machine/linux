...
probe() {
    ...
    tmux -L "$sock" kill-server 2>/dev/null
    # loaded if tmux lists the file, or if a setting only that file makes is present
    case "$loaded:$tpm_env" in
        *"$conf"*|*TMUX_PLUGIN_MANAGER_PATH=*) return 0 ;;
        *) return 1 ;;
    esac
}
if [ "$dry_run" = "0" ]; then
    if ! probe; then
        # OpenBSD's base tmux only reads /etc/tmux.conf and ~/.tmux.conf; the
        # ~/.config/tmux/tmux.conf location exists in portable tmux only. A
        # one-line ~/.tmux.conf that sources the real config covers both.
        if ! grep -qs 'config/tmux/tmux.conf' "$shim"; then
            log "tmux-setup: tmux ignores $conf (loaded: '${loaded:-none}'), adding a source-file line to $shim"
            printf '%s
' '# Generated by scripts/tmux-setup.sh: this tmux does not read ~/.config/tmux/tmux.conf'                 "source-file $conf" >> "$shim"
        fi
        probe || {
            {
                echo "tmux-setup: ERROR: tmux still did not load $conf"
                ...
                echo "  $shim:"; cat "$shim" 2>&1 | sed 's/^/    /'
            } >&2
            exit 1
        }
    fi
    log "tmux-setup: tmux loads $conf (prefix $prefix, $tpm_env)"
fi
